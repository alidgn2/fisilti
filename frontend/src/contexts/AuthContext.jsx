...
export function AuthProvider({ children }) {
    ...
    const register = async (email, password, name, consents = {}) => {
        const { data } = await api.post("/auth/register", { email, password, name, ...consents });
        setSessionToken(data.session_token);
        setUser(data.user);
        return data.user;
    };
    ...
    return (
        <AuthContext.Provider value={{ user, loading, login, register, logout, refresh: checkAuth, setUser, formatApiError }}>
            {children}
